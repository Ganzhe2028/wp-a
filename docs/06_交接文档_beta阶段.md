...
| ✅ 运营后台 | 批量导入、批量/单条填位置、下架、导出链接、重置密码、QR 码打印 |
...
前端 `browser-image-compression` 压缩 → 请求 `/api/upload-url` 拿 presigned PUT URL → 浏览器直接 PUT 到 R2 → 调 `/api/me/images` 存记录。头像上传传 `purpose: "avatar"`，不计入 4 张展示图限制；展示图保存必须使用 `/api/upload-url` 返回的完整 `key`。
...
### 6.3 登录闸不要去掉
`/u/[code]`、`/me` 页面开头都有 `verifyStudentSession()` 检查，无 session 会 redirect 到 `/?next=当前路径`。`/loc/[code]` 免登录可看位置，收藏按钮未登录时再跳登录并回到当前位置页。

### 6.4 状态 ≠ 页面空白
两个地方有特殊状态页：
- `person.hidden = true` → 显示「该页面已隐藏」占位
- 头像或姓名不足 → 显示「这位同学还没布置主页」占位
...
### 6.6 主页自动展示
学生保存头像 + 任一姓名后，服务端自动把主页标记为可展示。学生端不再需要发布开关；管理员只负责下架异常主页或图片。
...
### 6.11 生产密钥与登录限流
生产环境缺少足够长度的 `SESSION_SECRET` 或 `ADMIN_PASSWORD` 时，认证模块会直接报错，不再 fallback 到默认密钥。学生和管理员登录接口都有基础限流；仍建议在 Cloudflare/Vercel 层补边缘限流。
